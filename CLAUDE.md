...
### herdr sidebar command-status (2026-07-12)
`modules/shell.nix` (block trong `initContent`, chỉ chạy khi `$HERDR_PANE_ID`): hiện lệnh foreground đang chạy dở của mỗi space lên sidebar herdr qua zsh preexec/precmd hook (báo space "không sẵn sàng nhận lệnh").
...
- **Cắt custom_status ở 16 ký tự + `…`** (`_herdr_max`). `sidebar_width` của herdr mặc định **26 cột** (auto-scale 18–36, không phơi qua API) và herdr **cắt cụt KHÔNG thêm dấu gì** → chuỗi dài hơn bị xén mất luôn cái `…` của ta. Lệnh lỗi có tiền tố `✗` phải fit LẠI sau khi ghép.

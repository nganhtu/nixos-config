...
- `.config/fuzzel/` — fuzzel.ini + themes/noctalia. icon-theme Papirus. Font chính `Monaspace Xenon NF`, CJK fallback: `LXGW WenKai Mono` (Trung) → `Noto Sans CJK KR` (Hàn) → `Noto Sans CJK JP` (Nhật). Kitty dùng `symbol_map` ép dải Unicode: CJK→`LXGW WenKai Mono`, Hangul→`Noto Sans Mono CJK KR`, kana→`Noto Sans Mono CJK JP`, Hy Lạp+Cyrillic→`CaskaydiaCove Nerd Font Mono`. Han unification: kanji dùng chung codepoint chữ Hán nên hiện glyph kiểu Trung (LXGW), chỉ kana tách được sang font Nhật.
  - **fuzzel KHÔNG ép được theo dải như kitty** — nó chỉ có danh sách fallback tuần tự, font đứng trước mà có glyph là thắng. `LXGW WenKai Mono` phủ CẢ Hangul lẫn kana (nền Klee One) → 2 dòng Noto phía sau thực tế KHÔNG bao giờ tới lượt, Hàn/Nhật trong fuzzel ra nét bút LXGW chứ không giống kitty. Đảo Noto lên trước cũng không cứu được: Noto CJK KR phủ luôn chữ Hán nên sẽ cướp mất phần Hán. Đây là giới hạn của fuzzel, không phải cấu hình sai — đừng "sửa" bằng cách đổi thứ tự.
...
- **TODO_DIR BẮT BUỘC:** không set thì tuxedo mở `./todo.txt` theo cwd (không có → `/tmp/tuxedo-sample.txt`). Ép `home.sessionVariables.TODO_DIR = home` → luôn `~/todo.txt` + `~/done.txt` (cũng là 2 file được sync).
- **config.toml là app-TỰ-GHI** (theme/show_done/sort/filter/share-server) → KHÔNG symlink read-only (đóng băng hết state runtime). Chỉ pin vài key qua `home.activation` (sed in-place): `theme Bliss`, `show_done true`. nrs ghi đè lại key pin; runtime vẫn đổi được trong phiên (toggle H ẩn/hiện done), nrs kế đưa về pin.
- **done vs archived:** done = đánh dấu `x` (vẫn nằm trong todo.txt, ẩn nếu show_done=false, toggle H hiện lại); archived = dời hẳn sang done.txt (khỏi todo.txt), phải vào Archive view mới xem. App KHÔNG có unarchive.
- **Theme Bliss** (`themes/bliss.toml`) fork built-in "Terminal": theme file app chỉ ĐỌC → symlink read-only an toàn (khác config.toml). 25 field BẮT BUỘC đủ (không override một phần). Màu từ `palette.nix`. Gotcha: **dòng highlight dùng field `cursor`, KHÔNG phải `selected`** (`selected` chỉ cho multi-select/visual). Terminal gộp cursor+done vào chung ANSI 8 nên trông giống nhau; Bliss tách ra (cursor=`selectionBg`, done=`brightMagenta`).
- **Sync 2 máy qua gist** (`tuxedo-sync.nix`): systemd user timer 1 phút. gist (`nganhtu/4418…`) LÀ git repo. Script: flock → clone bootstrap về `~/.local/share/tuxedo-sync` → `.git/info/attributes` đặt `todo.txt done.txt merge=union` (giữ cả 2 phía khi conflict, không marker) → copy local vào repo, commit, fetch+merge origin/main (union), push HTTPS (askpass = `gh auth token --user nganhtu`, KHÔNG đổi account active) → ghi merged về $HOME atomically CHỈ khi local không đổi giữa chừng (race guard). tuxedo `write_atomic` + giữ thứ tự dòng → merge-friendly.
...
- **3 TRẠNG THÁI, phân biệt bằng termios + wchan** (watcher nền 2s/lần, chỉ gọi herdr khi state ĐỔI):
  - `idle` — tty ở chế độ **raw** (`-icanon`): một TUI đã chiếm terminal và đang sẵn sàng nhận phím (helix, tuxedo, btop). Nó KHÔNG chặn ta → space vẫn dùng được.
...
- **Chuỗi CJK fallback cho app fontconfig (2026-07-31):** `fonts.fontconfig.defaultFonts` ở `hosts/niquesse/configuration.nix` nối thêm CJK sau font Latin cho cả 3 generic — sans `Segoe UI Variable` → `LXGW WenKai` → `Noto Sans CJK KR` → `Noto Sans CJK JP`; mono `Monaspace Neon NF` → `LXGW WenKai Mono` → `Noto Sans Mono CJK KR/JP`; serif `Noto Serif` → `LXGW WenKai` → `Noto Serif CJK KR/JP`. Mục đích: GTK/Qt/browser hiện chữ Hán **cùng kiểu với kitty** (kitty đã ép `symbol_map` CJK→`LXGW WenKai Mono`). Font Latin PHẢI đứng đầu, không thì chữ Latin bị kéo sang LXGW.
  ...
  - **LXGW WenKai có sẵn cả Hangul lẫn kana** → đứng trước nên nó nuốt luôn 2 dải này, các entry `Noto Sans CJK KR/JP` phía sau thực tế không bao giờ tới lượt (giữ làm lưới an toàn). Muốn tiếng Hàn/Nhật ra đúng kiểu font bản địa thì phải đảo thứ tự, không phải chỉ thêm tên.

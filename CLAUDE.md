...
- `claude-desktop` → `github:aaddrick/claude-desktop-debian` (follows nixpkgs). Anthropic **không phát hành Claude Desktop cho Linux**; flake này tải `.deb` chính thức từ `downloads.claude.ai/claude-desktop/apt/stable` rồi autopatchelf — là app thật, không phải wrapper viết lại. Nạp qua `overlays.default` (đừng lấy `packages.*`: nó dựng nixpkgs riêng, nhân đôi closure). Hai package: **`claude-desktop`** (đang dùng) và `claude-desktop-fhs` — bản FHS thêm node/uv/docker/qemu/OVMF cho MCP server dùng đường `/usr/bin/...` và tính năng Cowork VM, đổi lấy vài GiB closure. MCP kiểu `npx` vẫn chạy ở bản thường vì `nodejs` đã có trong `systemPackages`. Version `.deb` ghim bằng hash → Anthropic ra bản mới thì `nix flake update` mới thấy.
  - **Phải truyền `--password-store=gnome-libsecret`** (2026-09-12), nếu không đăng nhập KHÔNG lưu được: *"Your sign-in won't be saved on this device."* Chromium tự chọn backend lưu mật khẩu bằng cách đọc `XDG_CURRENT_DESKTOP`/`DESKTOP_SESSION`; ở đây là `niri` (repo cố ý set vậy cho portal — mục 9c) nên không khớp GNOME/KDE, rơi về backend **`basic`** (plaintext) và `safeStorage` của Electron báo không khả dụng. **Không phải thiếu keyring** — đã kiểm: `gnome-keyring-daemon` chạy sẵn, `org.freedesktop.secrets` có mặt, `busctl --user get-property ... Locked` trả `false` cho cả collection `login` lẫn `default`. Lời khuyên trong thông báo ("cài + unlock keyring") đi sai hướng, đừng đuổi theo. **Không có biến môi trường nào thay được cờ này** → wrap binary ở let-binding `claude-desktop-keyring` (`hosts/niquesse/configuration.nix`, cùng kiểu `waydroid-fixed`) thay vì chỉ sửa `Exec=` của desktop entry — sửa desktop entry thì gọi từ terminal vẫn hỏng. Desktop entry upstream dùng `Exec=claude-desktop %U` (PATH-relative) nên tự ăn bản wrap.
  - Cùng cơ chế đó áp cho **mọi app Electron** trên máy này (VSCode, Discord, Figma…) — cái nào cần lưu credential mà báo lỗi keyring thì kiểm cờ này trước.
...
### herdr sidebar command-status (2026-07-12, initContent tách file 2026-07-17, herdr 0.7.4 + label động 2026-07-26, self-heal 2026-09-12)
...
- **Danh sách `_herdr_agents` phải đồng bộ với `herdr integration` mỗi lần nâng herdr.** Đã trôi một lần (kiểm 2026-09-12, herdr **0.9.0**): thiếu `qwen`, `grok`, `antigravity-cli` — mà `antigravity-cli` có cài trên máy này, chạy nó là shell `report-agent` đè lên record herdr tự quản → kẹt nhãn đúng như mô tả ở bullet "1 PANE CHỈ GIỮ 1 AGENT RECORD".
- **`_herdr_label` là cờ "còn nợ release", không phải chỉ là nhãn.** Chỉ xoá ở nhánh release thành công của precmd; mọi lối ra khác (nháy blocked, `elapsed < 1s`, release lỗi vì herdr chưa sẵn sàng) để nguyên, và `preexec` của lệnh KẾ TIẾP release nốt rồi mới bắt đầu lệnh mới. Không có lưới này thì **hụt release một lần là nhãn nằm lì trên sidebar mãi** — nhánh `elapsed < 1s` là ca có thật: lệnh dưới 1 giây mà watcher kịp report thì precmd bỏ qua release hoàn toàn. Release là idempotent nên gọi thừa vô hại.
...
- **Chỉ đặt tên ở hai chỗ có nguồn thật: `128+N` và `126/127`.** Tên signal lấy từ mảng `$signals` sẵn có của zsh, **lệch 1 index**: mã `N` → `$signals[N-127]`. Mảng này còn chứa **pseudo-signal của zsh** ở cuối (`EXIT`/`ZERR`/`DEBUG`) → phải lọc theo tên, không thì mã 191/192 (RT signal cao nhất) ra `(SIGZERR)`/`(SIGDEBUG)` sai. Chặn bằng cận trên số cứng cũng được nhưng bám vào build zsh cụ thể. **126/127 đáng tin vì do CHÍNH zsh sinh, không phải app** (`man zshmisc`: 127 = không tìm thấy, 126 = có file nhưng không exec được) — đã kiểm máy này không có `command_not_found_handler` nào chen vào đổi mã. Text nằm ở assoc `_natys_names` đầu file.
- **Mọi mã KHÁC đều để số trần, cố ý.** Dưới 128 không có không gian tên toàn cục: số chỉ có nghĩa tương đối với chương trình sinh ra nó. `2` là "sai cú pháp" với hầu hết app nhưng là "lỗi thật" với `grep`/`diff`; 64–78 (`sysexits.h`, có sẵn trong `glibc-dev/include`) chỉ đúng với app theo convention BSD (sendmail/postfix…) trong khi phần lớn tool hiện đại dùng 64+ cho nghĩa riêng → **đã cân nhắc và bác**, nhãn sai mà trông có thẩm quyền còn tệ hơn số trần. Cách phủ rộng hơn DUY NHẤT là bảng theo từng lệnh (`man` của chính tool có mục `EXIT VALUES`), khoá theo tên binary bắt ở `preexec` — chưa làm, và nếu làm phải bỏ qua pipeline vì `$?` là mã của phần tử CUỐI.
